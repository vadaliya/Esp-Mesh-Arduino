...
//  public broker address and port
...
void mqttCallback(char *topic, uint8_t *payload, unsigned int length)
{
    ...
    Serial.printf("Target String is: %s\n", targetStr.c_str());

    if (targetStr == "gateway")
    {
        Serial.printf("Enter into the Gatway for-loop\n");
        if (msg == "getNodes")
        {
            Serial.printf("Enter into getNodes\n");
    ...
    {
        Serial.printf("Enter into Brodacast\n");
        mesh.sendBroadcast(msg);
    }
    else
    {
        Serial.printf("Enter into Else loop\n");
